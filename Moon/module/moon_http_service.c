...
#define RECV_BUF_LENGTH 1024 //定义接收数据包的缓冲区大小
#define MIN_BUF 128 //最小缓冲区大小
#define RESPONSE_BUF_LENGTH 1024*10 //响应缓冲区大小
#define SEND_FILE_SIZE 1024 //每次发送文件的大小

	extern Moon_Server_Config* p_global_server_config;//服务配置全局变量

	static Array_List* pThreads = NULL;//线程池容器

	static bool g_bEndListen = false;//是否停止监听
...
/*****************************************************************************************************************/
	...
	{  
		...
		responseLength = moon_http_response(url,pResponseContent,sAccept);//如果有响应信息才发送数据包
		if (responseLength > 0)
		{
